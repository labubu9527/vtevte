 ...
 * @LastEditors: cdluxy
 * @LastEditTime: 2020-05-30 17:23:17
 */
...
const EditableCell = ({
	...
	const inputNode = inputType === 'select' ? (<Select defaultValue={record.role_name} style={{ width: 100 }}>
...
const UserManagement = ({ tableDataSource = [] }) => {
	...
	const syncRoleDesc = (rowData) => {
		...
		if(role_name === '管理员'){
			rowData.role_description = 'a.可查看并编辑所有系统中存在的项目；b. 可创建、编辑、删除用户账号，修改用户密码';
		}else if(role_name === '普通用户'){
			rowData.role_description = '仅可查看、编辑自己创建的项目';
		}
		return rowData;
	}
